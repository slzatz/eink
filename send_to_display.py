...
def create_palette_image():
    ...
    # Build palette data (256 RGB triplets, we only use first 6)
    palette_data = []
...
# Pre-create the palette image (used for all dithering operations)
...
def process_image(image_path, contrast=1.0, brightness=1.0, saturation=1.0, show_preview=False):
    ...
    # --- CONDITIONAL PREVIEW ---
    if show_preview:
        # Convert palette image back to RGB for preview
        preview_img = dithered.convert("RGB")
    ...
    # Get pixel data as flat list of palette indices
